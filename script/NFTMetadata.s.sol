...
contract NFTMetadataScript is Script {
    ...
    function run() external {
        ...
        bytes memory bytecode = type(NFTMetadata).creationCode;

        // Salt for deterministic address (can be changed)
        bytes32 salt = bytes32(uint256(0));

        // Deploy via CREATE2 proxy
        NFTMetadata nftMetadata = NFTMetadata(deployViaCreate2Proxy(bytecode, salt));

        console.log("=== NFTMetadata Deployment ===");
        console.log("NFTMetadata deployed at:", address(nftMetadata));
        console.log("==============================");
    ...
    function printAddress() external pure {
        ...
        console.log("NFTMetadata deterministic address:", predicted);
